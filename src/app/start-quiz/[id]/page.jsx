...
const Page = ({ params }) => {
  ...
  if (quiz.usersDone.find(i => i.id === user._id)) {
    ...
    <QuizDone questions={quiz.questions} name={quiz.name}
    userId={user._id} score={quiz.usersDone.find(u=> u.id === user._id)?.score} />
    </>;
  }

  return (
    <div>
      {quiz && quiz.questions.length > 0 && <Quiz questions={quiz.questions} name={quiz.name} quizId={quiz._id} userId={user._id} />}
    </div>
  );
};
...
const QuizDone = ({name,questions,userId,score}) =>{
  // console.log(questions[0].users.find(u => u.id === userId)?.choosed,questions[0].options.a,Object.keys(questions[0].options)[4])
   return <div className="px-4 sm:px-40 pt-12">
      ...
      {questions.map((question,index) =>{
        ...
        <div className="flex items-center gap-5">
          ...
          {!question.questionUrl?<p className="font-mono text-lg w-full sm:w-[80%]">
            {question.question}
        ...
        <div className={`flex ${question.questionUrl || 'flex-col'} flex-wrap justify-evenly gap-5 pt-8 px-4 sm:px-10 text-xl`}>
          {Object.values(question.options).map(
            (option, i) => {
              ...
              <OptionDiv
                ...
                data={option}
                index={i + 1}
                isSelected={question.users.find(u => u.id === userId)?.choosed === Object.keys(questions[0].options)[i]}
                isRight={question.answer === Object.keys(questions[0].options)[i]}
    ...
    <div className="flex items-center gap-3">
      ...
      <div
        ...
        className={`border-2 flex justify-between items-center gap-3 cursor-pointer border-purple-600 w-full rounded-lg p-3 min-w-16 text-center ${
           isRight ? "bg-green-600/85" : isSelected && isNotAttemp? "bg-purple-600/85":isSelected && "bg-rose-600/85"
        }`}
      >
        {data}
        {isSelected && isRight ? <span>+1</span>:isSelected && isNotAttemp? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
  <path fillRule="evenodd" d="M5.47 5.47a.75.75 0 0 1 1.06 0L12 10.94l5.47-5.47a.75.75 0 1 1 1.06 1.06L13.06 12l5.47 5.47a.75.75 0 1 1-1.06 1.06L12 13.06l-5.47 5.47a.75.75 0 0 1-1.06-1.06L10.94 12 5.47 6.53a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
</svg>:isSelected && <span>-0.25</span>
}
